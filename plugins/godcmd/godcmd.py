...
class Godcmd(Plugin):
    def __init__(self):
        ...
        logger.info("[Godcmd] 插件已初始化，处理器已注册")

        # 添加额外的检查
        try:
            # 检查事件处理器是否正确注册
            if Event.ON_HANDLE_CONTEXT in self.handlers:
                logger.info("[Godcmd] 事件处理器注册成功")
            else:
                logger.error("[Godcmd] 事件处理器注册失败")
            
            # 检查配置是否正确加载
            logger.info(f"[Godcmd] 当前配置: password={'已设置' if self.password else '未设置'}, admin_users={self.admin_users}")
        except Exception as e:
            logger.error(f"[Godcmd] 初始化异常: {str(e)}")
        
        logger.info("[Godcmd] 插件初始化完成")

    def on_handle_context(self, e_context: EventContext):
        try:
            # 记录开始处理事件
            logger.info("[Godcmd] 开始处理事件")
            
            context_type = e_context["context"].type
            logger.info(f"[Godcmd] 事件类型: {context_type}")
            
            if context_type != ContextType.TEXT:
                logger.info("[Godcmd] 非文本事件，跳过处理")
                if not self.isrunning:
                    e_context.action = EventAction.BREAK_PASS
                return

            content = e_context["context"].content
            logger.info(f"[Godcmd] 收到消息: {content}")
            
            if content.startswith("#"):
                logger.info("[Godcmd] 检测到指令: %s" % content)
                if len(content) == 1:
                    reply = Reply()
            ...
            # 捕获处理过程中的所有异常
            logger.error(f"[Godcmd] 处理事件异常: {str(e)}")
            import traceback
            logger.error(f"[Godcmd] 异常详情: {traceback.format_exc()}")
